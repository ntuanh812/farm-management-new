...
import { LIFECYCLE, BARN_STATUS, ROLE, REPORT_STATUS } from "../config/constants.js";

export const pigsController = {
  // =========================================================
  // GET ALL PIGS
  // =========================================================
  getAllPigs: async (request, reply) => {
    try {
      const whereClause = {};
      ...
      const pigs = await prisma.pigs.findMany({
        ...
        include: {
          ...
          pig_reports: {
            where: { status: { in: [REPORT_STATUS.PENDING, REPORT_STATUS.IN_PROGRESS] } },
            take: 1,
          },
      ...
      return reply
        .status(500)
        .send({ success: false, message: "Không tải được danh sách lợn" });
    }
  },
  ...
  // GET PIG HISTORY
  // =========================================================
  getPigHistory: async (request, reply) => {
    try {
      ...
      // Tạo tập hợp tất cả các mã chuồng lợn đã từng ở
      const barnIds = [pig.barn_id];
      movements.forEach((m) => {
        if (m.from_barn_id) barnIds.push(m.from_barn_id);
        if (m.to_barn_id) barnIds.push(m.to_barn_id);
      });
      const uniqueBarnIds = [...new Set(barnIds)];

      // Hàm helper kiểm tra xem lợn có ở trong chuồng tại thời điểm diễn ra sự kiện hay không
      const wasPigInBarn = (barnId, eventDate) => {
        const eventTime = new Date(eventDate).getTime();
        const entryTime = new Date(pig.entry_date).getTime();
        if (eventTime < entryTime) return false;

        // Sắp xếp di chuyển theo thứ tự thời gian tăng dần
        const sortedMoves = [...movements].sort(
          (a, b) => new Date(a.move_date).getTime() - new Date(b.move_date).getTime()
        );

        if (sortedMoves.length === 0) {
          return Number(barnId) === Number(pig.barn_id);
        }

        // Tìm lượt di chuyển đầu tiên diễn ra SAU ngày sự kiện
        const nextMove = sortedMoves.find(
          (m) => new Date(m.move_date).getTime() > eventTime
        );

        if (nextMove) {
          // Tại thời điểm eventTime, lợn vẫn ở chuồng xuất phát của lượt di chuyển kế tiếp
          return Number(barnId) === Number(nextMove.from_barn_id);
        } else {
          // Không có lượt di chuyển nào sau ngày sự kiện -> lợn đang ở chuồng hiện tại
          return Number(barnId) === Number(pig.barn_id);
        }
      };

      // 3. Lịch sử tiêm phòng (vaccinations)
      const vaccinationWhere = {
        OR: [
          { pig_id: pigId },
          { AND: [{ barn_id: { in: uniqueBarnIds } }, { pig_id: null }] },
        ],
      };
      ...
      // Lọc danh sách tiêm phòng: nếu tiêm cho cả chuồng, lợn phải ở trong chuồng tại ngày tiêm
      const filteredVaccinations = vaccinations.filter((v) => {
        if (v.pig_id !== null) return true; // Tiêm riêng cho con này
        return wasPigInBarn(v.barn_id, v.vaccinated_at);
      });

      // 4. Lịch sử dùng thuốc (medicines)
      const medicineWhere = {
        OR: [
          { pig_id: pigId },
          { AND: [{ barn_id: { in: uniqueBarnIds } }, { pig_id: null }] },
      ...
      // Lọc danh sách thuốc: nếu dùng cho cả chuồng, lợn phải ở trong chuồng tại ngày dùng thuốc
      const filteredMedicines = medicines.filter((m) => {
        if (m.pig_id !== null) return true; // Dùng riêng cho con này
        return wasPigInBarn(m.barn_id, m.used_at);
      });
      ...
      const data = {
        ...
        vaccinations: filteredVaccinations.map((v) => ({
          id: v.id,
        ...
        movements: movements.map((m) => ({
          ...
          from_barn_name:
            m.barns_pig_movements_from_barn_idTobarns?.name,
      ...
      return reply
        .status(500)
        .send({ success: false, message: "Không tải được lịch sử cá thể lợn" });
    }
  },
  ...
  createPig: async (request, reply) => {
    try {
      ...
      if (!barn_id || !category || !entry_date) {
        return reply
          .status(400)
          .send({ success: false, message: "Thiếu dữ liệu bắt buộc" });
      }
      ...
      return reply.send({ success: true, message: "Nhập lợn thành công" });
    } catch (err) {
      console.error(err);
      return reply
        .status(500)
        .send({ success: false, message: "Không thể nhập lợn" });
    }
  },
  ...
  updatePig: async (request, reply) => {
    try {
      ...
      if (oldStatus === LIFECYCLE.DEAD || oldStatus === LIFECYCLE.SOLD) {
        return reply.status(400).send({
          success: false,
          message:
      ...
      if (oldBarnId && Number(barn_id) !== oldBarnId) {
        const newBarn = await prisma.barns.findUnique({
          ...
          include: {
            _count: {
              select: {
                pigs: { where: { lifecycle_status: LIFECYCLE.ACTIVE } },
              },
            },
          },
        });
        ...
        if (newBarn.status === BARN_STATUS.MAINTENANCE) {
          return reply.status(400).send({
            success: false,
      ...
      return reply.send({ success: true, message: "Cập nhật lợn thành công" });
    } catch (err) {
      console.error(err);
      return reply
        .status(500)
        .send({ success: false, message: "Không thể cập nhật lợn" });
    }
  },
  ...
  deletePig: async (request, reply) => {
    try {
      ...
      if (
        pigInfo &&
        (pigInfo.lifecycle_status === LIFECYCLE.SOLD ||
          pigInfo.lifecycle_status === LIFECYCLE.DEAD)
      ) {
        return reply.status(400).send({
          success: false,
          message:
      ...
      await prisma.pigs.delete({ where: { id: Number(id) } });

      return reply.send({ success: true, message: "Xóa lợn thành công" });
    } catch (err) {
      console.error(err);
      return reply
        .status(500)
        .send({ success: false, message: "Không thể xóa lợn" });
    }
  },
};
