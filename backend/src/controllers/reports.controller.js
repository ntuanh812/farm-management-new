...
import { LIFECYCLE, BARN_STATUS, REPORT_STATUS } from "../config/constants.js";
  ...
  getOverview: async (request, reply) => {
    try {
      ...
      const activePigsAgg = await prisma.pigs.groupBy({
        by: ["category"],
        _count: { _all: true },
        where: { lifecycle_status: LIFECYCLE.ACTIVE },
      });
      ...
      const barnAgg = await prisma.barns.aggregate({
        _count: { _all: true },
        _sum: { capacity: true },
        where: { status: { not: BARN_STATUS.MAINTENANCE } },
      });
      const barn_stats = {
      ...
      const feedsInfo = await prisma.feeds.findMany({
        where: { id: { in: feedIds } },
      });
      ...
      const vaccinesInfo = await prisma.vaccines.findMany({
        where: { id: { in: vacIds } },
      });
      const vacMap = Object.fromEntries(
        vaccinesInfo.map((v) => [v.id, v.name]),
      );

      const vaccine_stats = vacAgg
        .map((v, index) => ({
          vaccine_name: vacMap[v.vaccine_id] || "Không xác định",
          total_quantity: Number(v._sum.quantity || 0),
          unit: v.unit || "",
        }))
      ...
      const medicinesInfo = await prisma.medicines.findMany({
        where: { id: { in: medIds } },
      });
      const medMap = Object.fromEntries(
        medicinesInfo.map((m) => [m.id, m.name]),
      );

      const medicine_usage = medAgg
        .map((m) => ({
      ...
      return reply
        .code(500)
        .send({ success: false, message: "Lỗi tải báo cáo tổng quan" });
    }
  },
};
